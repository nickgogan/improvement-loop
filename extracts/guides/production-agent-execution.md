---
title: "Production Agent Execution"
type: "guideline"
...
tags:
  ...
  - "operations"
contract:
  preconditions: "Agent architecture chosen (see G3); moving to supervised production execution design"
  invariants: "Planning is probabilistic; execution is deterministic; every workflow is observable, state-tracked, and recoverable; delivery is calibrated to reviewer capacity"
  governance: "IL-owned draft; Nick deploys"
  recovery: "If workflows fail silently, add observability first; if costs explode, add budgets; if crashes lose state, add persistence; if review is the bottleneck, build quality at source"
---

# Production Agent Execution

You have chosen your agent architecture (see *Agent Architecture Decisions*, G3). Now: how do you run it reliably in production under human supervision? How do you separate planning from execution, persist state across crashes, observe what agents are doing, control costs, handle failures gracefully, deliver work at a pace humans can review, and route incoming work to the right workflow?

This guide covers the "how" of supervised agent operations — workflow engines, execution patterns, observability, cost management, degradation modes, delivery cadence, and workflow routing. It is the production-hardening companion to G3's architectural choices. For running agents *unattended* — scheduled, headless, and autonomous — see *Autonomous and Scheduled Agent Operation* (G3d), the sibling this guide was split from (DD-122).
...
**Do not use for:** running agents unattended, on schedules, or fully autonomously (see G3d); choosing between single-agent and multi-agent (see G3); specifying what an agent does (see G1); managing context within an agent (see G2); maintaining memory and session continuity (see G7); or evaluating agent output quality (see G4).
...
> For pipelines whose work items already live in an external tracker (GitHub issues, tickets), workflow state can be encoded as *labels on the work item itself* — a pattern that also gives an autonomous orchestrator free double-dispatch protection and free observability. That label-as-state-machine design is covered in *Autonomous and Scheduled Agent Operation* (G3d), where it anchors the unattended-pipeline state model.
...
Without observability, debugging agent systems requires reading raw conversation logs. Build structured observability at two levels for a single supervised workflow: real-time events within a run, and post-hoc traces across a run. (Once you run *recurring* scheduled loops, a third level -- portfolio health across all runs -- is required; that management layer lives in G3d.)
...
**Stall detection:** Monitor revision loops for plateauing issue counts across iterations. An agent stuck at 5 issues for 3 iterations is stalled even though 5 is below most absolute thresholds. Combine trajectory monitoring (catches subtle stalls), hard stop gates (catches everything), and consecutive-call guards (prevents runaway execution) for defense in depth. In multi-agent pipelines, also watch for handoff stalls: an agent waiting on input from a crashed upstream agent waits forever -- detect these with per-stage timeouts.

> Unattended pipelines add two more cost controls that are existential rather than optional -- a per-cycle batch cap and an explicit rate-limit pause state. Those live in G3d, Step 6, because a misconfigured cron job is an unbounded liability in a way an interactive session is not.
...
**The key insight:** Most agent failures in production are not binary (works / does not work). They are degraded states where the agent produces plausible but incorrect output. Explicit degradation modes make failure visible and controlled rather than silent and compounding. (In *unattended* operation, "human escalation" needs a concrete trigger and a retry ceiling -- that escalation-ladder discipline is in G3d, Step 7.)
...
### Step 9: Route Work to the Right Workflow
...
This template covers the supervised-execution surface. If the workflow is also scheduled or autonomous, fill the Scheduling and Autonomy sections from the corresponding template in G3d.
...
- State store: {{STATE_FILE/DATABASE/ENGINE_NATIVE}}
...
### 9. Routing by memory instead of by description
...
- **Unattended operation ->:** *Autonomous and Scheduled Agent Operation* (G3d) is this guide's sibling — it covers scheduling surfaces, headless composition, loop contracts, and autonomy levels. The shared substrate (durable execution, cost caps, degradation modes, YAML DAGs) appears in both.
...
- **Observability -> context engineering:** The tracing and event patterns (Step 5) complement context management strategies in *Managing Agent Context* (G2a/G2b).
...
- The workflow involves side effects (file writes, API calls, deployments) or runs longer than a single interactive turn under human supervision.
...
- Cost controls (budgets, pre-turn projection, termination rules, caching, fast-fail) are in place before execution scales.
...
- This guide is owned by the IL knowledge layer and updated when new supervised-execution findings are integrated.

### Recovery
- If workflows fail silently: add observability first (Step 5). You cannot fix what you cannot see.
- If costs explode: add per-step budgets, pre-turn projection, and loop termination rules (Step 6). Check for stall loops and Opus overuse.
...
- If the wrong workflow runs for a task: implement complexity tiering (Step 3) and deliberate routing (Step 9).
- If you need to run any of this unattended: switch to G3d and climb the autonomy ladder deliberately.
