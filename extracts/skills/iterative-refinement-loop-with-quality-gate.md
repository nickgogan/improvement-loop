...
context:
  applies_to:
    ...
    - "workflows that can afford a separate evaluator role to remove self-scoring bias"
  ...
  adoption:
    ...
    notes: "Claude Code uses the single-agent loop natively for code review; Archon v0.5.0 ships the two-role adversarial variant (Generator + Evaluator, 7/10 numeric gate, bounded retries) as a default product workflow."
contract:
  preconditions: "A scoring rubric with named criteria and explicit pass thresholds is available before invocation. The caller has defined the scale (e.g., 1–5 or 1–10) and the max iteration cap. The task is well-specified enough that the agent can produce a meaningful first draft. The rubric criteria are specific enough to generate actionable failure reasons."
  invariants: "The loop terminates: either all criteria pass at or above threshold, or the max iteration cap is reached. The iteration log is produced regardless of pass/fail outcome. The pass/fail verdict is explicit — the skill does not self-report \"pass\" without all criteria meeting threshold. Criteria definitions do not change between iterations within a single invocation. When the adversarial variant is used, the evaluator is a distinct role from the generator and its verdict is authoritative."
...
- **Scale definition:** The numeric scale used for scoring (e.g., 1–5, where 1 = does not meet criterion, 5 = fully meets criterion; or 1–10 with a 7 gate, as Archon ships).
- **Max iterations:** An integer cap on the number of refinement loops (recommended default: 3). Prevents runaway token consumption.
- **Optional — evaluator role:** A separate agent/role that scores the draft, distinct from the generator (see the Adversarial Variant below). Removes self-scoring bias at the cost of a second role.
...
4. **If any criterion fails:** identify the specific failure reasons for all failing criteria. Rewrite the draft targeting only the failing criteria — do not regress passing criteria. In the adversarial variant, the evaluator's per-criterion feedback is handed back to the generator as the rewrite brief.
...
## Adversarial Generator/Evaluator Variant (Archon v0.5.0)

The strongest mitigation for self-evaluation bias is to split scoring away from the generating agent entirely. Archon v0.5.0 ships this as a default product workflow (`archon-adversarial-dev.yaml`): a Planner sets up the work, then a state-machine loop alternates two roles:

- **Generator** produces the draft (and each rewrite).
- **Evaluator** is a distinct adversarial role whose job is to *break* the generator's work against explicit numeric criteria. Any criterion scoring below **7/10** sends the work back to the generator with the evaluator's feedback, under **bounded retries** (the max-iteration cap).

Because the scorer never re-reads its own generation as author, the "model is lenient toward its own output" failure mode is answered structurally, not just by prompt discipline. Use this variant whenever the workflow can afford a second role and the output is high-stakes; fall back to the single-agent loop (Steps 1–6) when a separate evaluator is too costly, accepting the self-scoring caveat below.

## Failure Modes

- **Self-evaluation bias.** The same model that generates the output also scores it. Models may be systematically lenient toward their own outputs, producing inflated scores that mask real quality gaps. Mitigation (weak): use externally defined, specific criteria with concrete examples of each score level, and a separate evaluation pass instructed to score critically. Mitigation (strong): adopt the adversarial Generator/Evaluator variant above — a distinct evaluator role removes the bias structurally, as shipped in Archon v0.5.0.
...
### Preconditions
A scoring rubric with named criteria and explicit pass thresholds is available before invocation. The caller has defined the scale (e.g., 1–5 or 1–10) and the max iteration cap. The task is well-specified enough that the agent can produce a meaningful first draft. The rubric criteria are specific enough to generate actionable failure reasons.
