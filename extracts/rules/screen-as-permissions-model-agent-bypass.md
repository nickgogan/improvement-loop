...
The Lilly/McKinsey incident demonstrated this failure mode at production scale: 22 of 200 API endpoints were unauthenticated, including endpoints with production write access. A $20 autonomous agent obtained read/write access to data used by 70% of 40,000 consultants. This is not an edge case — as of early 2026, autonomous agents probing public endpoints for production data is described as "very normal." The failure is systemic: APIs designed under the assumption that only humans reach them via a UI are inherently unsafe for agentic consumption. The UI was the security boundary; agents have no UI.
