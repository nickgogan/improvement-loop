...
- Tier-3 reads leave a Librarian trace via SL entry (per Nick's decision to use SL for usage-record responsibility). One-line SL entry is enough: `Librarian Tier-3 read: <watched-lib/path>:<range> — reason: <consumer ask>`.
...
- **SL as usage record.** Per Nick's session-49 gate: the IL System Log carries Librarian-usage-record responsibility. Tier-3 reads and other cost-bearing operations emit a one-line SL note. No separate usage log.
...
2. **SL entry shape for Tier-3 reads.** One-line notes seem right. Do you want structured frontmatter or free-form appended lines under a rolling `librarian-reads` SL entry?
