...
| DD-99 | Theme graduation procedure (PROMOTE path). Detection occurs at `/identify-artifacts` Step 6.b (Unrouted Bucket review); this skill is the downstream executor when the per-graduation DD lands. PROMOTE path's new-guide first changelog entry uses the new `theme-graduation` trigger tag (per IB-160 enum amendment to DD-94). Body line: "First synthesis from graduated theme [[<theme-name>]] per [[DD-XXX]]" where DD-XXX is the per-graduation DD. ABSORB path (handled by `/dimension-rebalance` + destination guide's next regen) uses the existing `dimension-rebalance` tag — no amendment needed. Step 4.5 enum extended to accept `theme-graduation`. |
