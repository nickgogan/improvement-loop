...
| `--session NN` | Active IL session number for the lifecycle-pointer fields (DD-117). Required on every write of a non-guide artifact (rule, skill, template, agent). Skill prompts if missing (unless `--auto`, which aborts). Ignored for guide writes (this skill does not write guides; defensive). The report half of the lifecycle pointer (`last_change_report`) takes no argument — it is derived from the run's authorizing input (identification report or harvest-queue file) per DD-117. |
| `--update` | Re-extraction mode (DD-117). When set, the skill OVERWRITES existing artifacts whose `source_finding` matches a finding in the current run, rather than skipping them per the default dedup-at-write rule. Triggers: post-drift-report Nick ruling (DD-96), Nick-requested re-extraction, dimension-rebalance regen. On update, body + ContractSpec + ContextSpec are regenerated; `last_change_session` + `last_change_report` are overwritten with the current run's values (a legacy `last_change_sl` field, if present, is removed in the same write); `extraction_date` is preserved (the original extraction date — not the update date); `deployed` + `deployed_to` are preserved. |
...
(d) **No in-place version overwrite.** Once `<name>-v<N>.md` exists, `/extract-artifacts` NEVER overwrites it. Subsequent regen of an even-newer version produces `<name>-v<N+1>.md`, never an in-place rewrite of any prior version. The `--update` flag (DD-117 re-extraction path) does NOT apply to versioned files — `--update` overwrites the unsuffixed baseline only. A `--version-bump`+`--update` combination is ill-formed; the skill aborts with a structured error.

(e) **Full independent frontmatter on every version.** Each version carries its own `source_finding` (typically the version-bumping finding; can differ by Nick direction), own `extraction_date` (current write date; never inherited), own `last_change_session` + `last_change_report` (per DD-117), own `contract` (DD-78), own `context` (DD-92), own `version: <N>` (per DD-100 + IB-161 schema field). Cross-version frontmatter inheritance is forbidden — every version writes fresh from the current run's resolved values.
...
**Two distinct subagent patterns exist around this skill — disambiguate.**

| Pattern | Where it runs | What the subagent does | Step 4.8 ownership |
|---|---|---|---|
| **Drafting subagent (this Step 2)** | Inside a single `/extract-artifacts` invocation (identification-report mode); the skill itself launches per-batch drafters to produce JSON output | Drafts artifact body + ContractSpec + ContextSpec; outputs JSON; **does not write files**; **does not touch queue files** | Skill orchestrator (single process; all writes happen in Steps 3–5 of the same invocation) |
| **Whole-skill-invocation-as-subagent (harvest-mode parallel batches)** | A higher-level orchestrator (e.g., a Codifier session running a bulk promotion) spawns N parallel subagent processes, each invoking `/extract-artifacts --harvest-row <id-i>` end-to-end as its own task | Runs the **full** harvest-mode pipeline: Step 0a → 1 → 1.7/1.8 → 2 (drafting may happen in-process given single-row scope) → 2.5 → 2.7 → 3 → 4 → **4.8** → 5 | The spawned skill instance (each `/extract-artifacts` invocation owns its own Step 4.8 per the skill contract; the higher-level orchestrator MUST NOT post-batch-write the queue file) |

The drafting-subagent pattern (this section) applies only to identification-report mode. Harvest-mode (`--harvest-row` / `--harvest-dismiss`) is single-row by Step 0a step 7's contract; the per-row drafting may run in-process without further subagents, or a higher-level orchestrator may spawn per-row whole-skill subagents — but in either case the skill's own pipeline is end-to-end (no orchestrator-side queue write-back). See Step 4.8's **End-to-end execution invariant** and **Concurrency for parallel harvest-row batches** for the harvest-mode subagent contract and partition-by-queue-file rule.
...
### Step 2.7: Resolve Lifecycle Pointer (DD-117) — non-guide artifacts only

Every non-guide artifact (rule, skill, template, agent) carries two required frontmatter fields per DD-117:

- `last_change_session: <integer>` — the active IL session number.
- `last_change_report: "<report-filename-stem>"` — the bare filename stem of the run's authorizing input (no path, no `.md` extension).

This step resolves both values before any artifact is written. The session number is an explicit input (never auto-derived from git or filesystem state); the report stem is derived from the run's own input.
...
2. **Derive report stem from the run's authorizing input.**
   - **Report mode** (`<report-filename>` / `--latest`): the identification report resolved in Step 0 — strip path and `.md`, store the bare stem.
   - **Harvest mode** (`--harvest-row` / `--harvest-dismiss`): the harvest-queue file resolved in Step 0a — strip path and `.md`, store the bare stem (e.g., `<guide-stem>.harvest-queue`).
   - No argument, no prompt, no separate existence check — the input's existence was already validated when the run opened it. A run that reached this step has its anchor in hand by construction (DD-117).

3. **Hold for Step 3.** Cache `(active_session, active_report_stem)` for use during the per-artifact write loop. Both fields are written verbatim into the frontmatter of every non-guide artifact created or updated in this run. If an existing artifact being updated carries the legacy `last_change_sl` field (DD-95-era; a valid read-only pointer into the frozen SL corpus), remove it in the same write.
...
2. **Check for filename collision and dedup behavior** in the target directory:
   ...
   - **Update mode (`--update`):** if an existing artifact has the same `source_finding`, this is a re-extraction. Read the existing artifact's frontmatter to capture preserve-on-update fields: `extraction_date`, `deployed`, `deployed_to`. The new body + ContractSpec + ContextSpec replace the existing ones. The two preserved fields are written back verbatim. `last_change_session` + `last_change_report` are overwritten with the active run's values from Step 2.7 (no preservation; git carries the prior pointer), and any legacy `last_change_sl` field is removed. Filename is unchanged. `--update` does NOT apply to versioned writes — see versioned-write branch below.
...
last_change_session: [active session integer — DD-117]
last_change_report: "[authorizing report/queue stem — DD-117]"
...
**End-to-end execution invariant.** Step 4.8 runs in the **same process** as the artifact write (Step 3) and the source back-annotation (Step 4). The harvest-mode pipeline — Step 0a → 1 → 1.7/1.8 → 2 → 2.5 → 2.7 → 3 → 4 → 4.8 → 5 — is a single end-to-end skill invocation per row. This preserves atomicity from artifact write through queue flip: the artifact's existence on disk is the precondition for the queue update, and both happen within one skill execution. An orchestrator that spawns `/extract-artifacts --harvest-row` invocations as subagents MUST NOT itself write the queue file post-batch — that contradicts the skill contract, splits atomicity across processes, and creates orchestrator-vs-subagent contention. The skill — wherever it runs — owns Step 4.8.

**Inputs.** The cached `harvest_row_id` from Step 0a (compound `<finding-stem>::<target-form>::<headline-slug>`) and the cached resolved queue file path (from Step 0a step 2 auto-locate); the active `(active_session, active_report_stem)` pair from Step 2.7 (cached even though no DD-117 artifact-write occurs in dismiss mode — the report stem in either harvest mode is the resolved queue file's own stem; in dismiss mode the session number and queue stem annotate the queue-row footer); the outcome of Steps 1.7 / 1.8 / 3 (whether an artifact was written, whether an extension or version-bump proposal was emitted, or no-op).
...
**Concurrency for parallel harvest-row batches.** When an orchestrator spawns multiple `/extract-artifacts --harvest-row` invocations in parallel (one per row), the parallel batch MUST be partitioned by queue file: **at most one active invocation per `extracts/guides/<stem>.harvest-queue.md` at any moment**. Within a single queue file, rows are processed sequentially; across queue files, parallelism is unbounded. Rationale: each invocation's Step 4.8 is a per-process atomic read-modify-write of the entire queue file. Two concurrent invocations targeting the same queue file race — both read the pre-state, both modify their respective rows in their in-memory copies, both write the whole file back; last-writer-wins, the loser's row update is silently lost. Partitioning eliminates the race by serializing all writers to a given queue file. The skill itself does not enforce this — it is the orchestrator's contract when batching. Single-invocation use (no parallel batching) and parallel batching across distinct queue files are both unconstrained by this rule. Session 83 surfaced the latent failure mode: when the orchestrator's prompt instructed subagents to skip Step 4.8 (intending to batch queue updates post-subagent-completion), some subagents complied and others honored the skill contract; the orchestrator-batched updater used "match-then-skip" recovery semantics that masked the divergence as a non-issue. The partition rule above eliminates the need for any such recovery.
...
4. **Dedup at write time.** Check if an artifact for this finding already exists in `extracts/`. In default mode, skip. In `--update` mode (DD-117 re-extraction path: post-drift-report ruling, Nick-requested re-extraction, dimension-rebalance), overwrite the existing artifact per Step 3 update-mode rules — body + ContractSpec + ContextSpec regenerated; `extraction_date`, `deployed`, `deployed_to` preserved; `last_change_session` + `last_change_report` overwritten with current run values (legacy `last_change_sl` removed).
...
| `--version-bump` combined with `--update` | Step 3 versioned-write branch ill-formed-combination check | Abort with structured error. `--update` overwrites the unsuffixed baseline (DD-117 re-extraction); versioned writes produce a new sibling file (DD-100 version bump). The combination is structurally incoherent. |
...
| DD-117 | Lifecycle pointer (`last_change_session` + `last_change_report`) on every non-guide create AND update; report stem derived from the run's authorizing input (identification report / harvest-queue file — no `--sl` argument, no SL validation); both fields overwritten on update, legacy `last_change_sl` removed on update; guides excluded. Supersedes DD-95 after the SL producer retirement (DD-116). Step 2.7, Step 3 frontmatter template, Step 3 update-mode dedup behavior. |
