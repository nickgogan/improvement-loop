...
| [Harness](harness.md) | — | The whole-system shape the engine's top altitude composes and audits — a fractal-unit system composing skills/agents/prompts. The unit `/audit-artifacts` audits and `/design-harness` constructs. (The runtime sense — the surface an agent runs *inside* — is [Runtime Environment](runtime-environment.md).) |
...
| [Runtime Environment](runtime-environment.md) | — | The runtime + tooling surface an agent operates *inside* — CLI/IDE/API, distinct from the agent's own prompt. The runtime sense of "harness"; supplies the audit-time §Composition substrate. (The whole-system sense is [Harness](harness.md).) |
