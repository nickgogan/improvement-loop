...
tags:
  - "librarian-concept"
  - "runtime-environment"
  - "harness-runtime-sense"
  - "cross-cutting"
aliases:
  - "Runtime environment"
  - "Agent harness (runtime sense)"
  - "Runtime harness"
---

# Runtime Environment (the runtime sense of "harness")

> **Name note (DD-111 / IB-170, session 126):** This concept was formerly filed as `harness.md`. It is
> renamed to `runtime-environment.md` to resolve a name collision: "harness" also denotes the
> **whole-system unit** the engine's top altitude composes and audits (`harness.md`, this directory).
> This file is the **middle-altitude runtime sense** — the surface an agent runs *inside* — and supplies
> the audit-time §Composition substrate for harness/runtime questions. See `harness.md` for the
> whole-system unit.

## Short definition

The **runtime environment** (the runtime sense of "harness") is the runtime + tooling surface an agent operates *inside*: the CLI/IDE/API that loads the agent's prompt, enforces its permissions, invokes its tools, manages its context window, and executes its hooks. The agent is *in* the runtime environment; the runtime environment is not *in* the agent.
