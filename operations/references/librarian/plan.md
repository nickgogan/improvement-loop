...
| **Build** | Implement against the spec — context files, prompt composition, tool registry, memory wiring, governance scaffolding. | Variant-scoped guide union's `### Procedure` / `### Step N` sections. Prompt-based agent variant: G2 + G8. Harness-based variant: add G3 + G3b + G5 + G6. Autonomous-vs-supervised: add G7 + G9. |
