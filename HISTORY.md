...
## Session 152 — 2026-07-19 — guide refresh + splits + harvest-queue drain (753517e…3843201)

**Outcome:** ruled-queue guide refresh shipped and then, under two successive Nick trust grants, the whole downstream chain cleared in one session: six guides re-synthesized from wave-4 + the memory-spec corpus, both proceed-recommended DD-98 splits executed, and the entire DD-101 harvest-queue backlog drained — 79 rows into 67 staged artifacts with all nine corpus-overlap rulings made and applied. The extracts substrate is fully caught up with the KB for the first time since the queues were introduced.

- **Guide refresh (753517e):** six re-syntheses via `/synthesize-guide` (Opus subagents, serial; `--trigger nick-request --session 152`) — G7 35→40, G3 65→72, G3b 37→42, G11 37→39, plus Nick-added threshold-crossers G4 73→77 and G10 44→47. The memory-spec B-synthesis steered G7 (contested-axis verdicts: gated>autonomous promotion, keyword-first recall; failure-catalog pitfalls; disk-persistence substrate step) and the C-synthesis steered G3/G3b (six harness dimensions, enforcement-locus consensus, scheduling gap) — corpus cited in prose by path only, never in `source_findings` (the one ruled deviation from the skill's KB-only default). DD-93 was a no-op corpus-wide (no preserved surfaces exist yet); all six DD-94 changelogs clean.
- **Splits (ec4b146):** the two second-consecutive-firing proceed proposals executed per DD-98 — **DD-122**: G3b → G3c `production-agent-execution` (27) + G3d `autonomous-scheduled-agent-operation` (20); **DD-123**: G4 → G4a `verifying-agent-output` (63) + G4b `eval-driven-improvement-loops` (17). Sources deprecated + archived per the G2 precedent; harvest queues transferred per-row; routing table updated across all sections; 119 findings' `consumed_by` re-pointed with zero residual refs. The four defer-recommended proposals (G7/G3/G10/G11) stand as emitted.
- **Harvest-queue drain (dd9266b):** Nick's blanket per-recommendation ruling over all 79 live rows; two-wave execution partitioned by queue file (14 Sonnet lanes for rule/template volume, 9 Opus lanes for skill-form/merge/dup rows). Yield: **62 new staged artifacts** (36 rules, 19 templates, 7 skills), 4 inline dismissals, 3 idempotent recoveries, 1 split-inherited duplicate resolved; 9 rows routed to DD-97/DD-100 proposals instead of near-duplicates.
- **Corpus-overlap rulings (3843201):** all nine delegated by Nick and ruled concurring with the Codifier: 5 create-new (deterministic-execution-boundary-rule, cross-provider-validator-assignment-rule, headless-output-verifiability-gate, four-estimate-routing-test-skill, loop-contract-file-schema as unsuffixed baseline — each cross-referencing its family sibling) + 4 extend-existing merges (trigger/wake special case → deterministic-nodes rule; statefulness carve-out → CLI-first rule; mode-as-callable-tool → prefix-stability rule; garbage-collection-day evidence → recurring-review-comment rule). Rulings tables appended to both 2026-07-19 proposals reports. A suspected twin rule proved to be a citation typo in the proposal, not a corpus duplicate.
- **Lesson L-15:** Step 4.8 queue write-back left Status inconsistent with Resolution twice (s146 drift + a s152 lane); post-batch deterministic consistency check is now the stated rule.

## Session 151 — 2026-07-18→19 — link-intake wave-4 end-to-end: triage + Pass 2 into the KB (6a34195)

**Outcome:** ruled-queue item 1 closed in one session — the full 30-link LINKS.md batch (29 videos + langchain-ai/openwiki) triaged, Nick-accepted, and Pass-2-extracted into the KB behind the G9.I6 batch gate; both Nick inline notes resolved (vercel/eve registered as a watched library; mattpocock video needed no re-analysis); LINKS.md is empty for the first time since wave-2.

- **Triage** (4 topical Sonnet subagents, read-only; model class Nick-confirmed pre-fan-out): **1 ADD / 4 ENHANCE / 13 KB-ONLY / 12 REJECT / 0 defer**. The ADD: Geoffrey Litt's quiz-gated comprehension-gate skill (ExplainDiff gist verified) aimed at the prose-heavy-DD gating problem. The ENHANCEs target `/meta-skill-author` (Schmid evals anchor — queue-item-3 feedstock), `/self-improve` (WorkOS 77%→97% skill-regression case, transcript-mining, proof-of-execution), `/dd` (lint-enforced decision scoping, executable behavior specs), `/simplify-context` (A/B-before-cutting; per-control record seeds the asset-description schema). Report: `operations/research-reports/2026-07-18-link-intake-triage.md` (verdicts, rationales, calibration, cost table, follow-up queue).
- **Transcripts 29/29, zero defers:** 20 via api; mid-run YouTube IP block (429) took 7, all recovered via the kome.ai fallback lane — the PROGRESS backlog trigger fired on its second proven occurrence → **IB-180** filed (build kome into fetch.py; chunking mitigates but doesn't prevent).
- **Pass 2** (6 staged Sonnet extractors, scratchpad staging, live KB read-only until the gate; one batch approval): **28 new findings** (2 P1 with live engine hooks: lint-failures-as-remediation-prompts → the pre-commit validator's messages; structured-handoff-schema → `agents/handoff-protocol.md`), **8 finding updates** — incl. the `harness-engineering-third-evolution` primary-source upgrade (Medium→Strong; Lopopolo originated the term) and the `persona-clone-review-board` cartridge delta carrying an explicitly flagged autonomy-vs-rehearsal tension between sources. **12 sources, 11 new + 2 updated authorities** (new `ai-engineer.md` conference entry consolidating 6 sources; sponsorship-calibration note added to `cole-medin.md`). All 53 staged files validator-clean pre-apply; zero slug collisions.
- **Watched libraries:** `eve.md` (vercel/eve, Apache-2.0, 3,853★ — folder-compiled-manifest is queue-item-2 prior art, incl. a deliberate `contradicts` link vs `machine-readable-system-contract-with-wiring-rows`) and `openwiki.md` (MIT, 12,280★ — CI-driven self-updating agent wikis, E1 reflection-loop prior art; its absence from the memory-spec 12-framework survey logged in the IB-178 ledger). `pi-agent.md` gained a Notable Extensions note (pi-agent-sub-agents). 11.A loop cluster += the two new loop findings.
- **Per-channel calibration** recorded in the report (feeds the authorities registry): AI Engineer strong on named-practitioner quantified talks; Nate B Jones split-lane confirmed; Skill Advancement = content-mill noise; Pocock walkthroughs of already-tracked stable releases = light-skim only.
- **Lessons:** L-13 (zsh word-splitting — two same-session Bash loop failures, one silently degenerating a dedup check) and L-14 (Pass 2 planning by intake-path label missed the Eve video's extractable patterns; rule: novel-pattern estimate ≥1 ⇒ extraction slot regardless of path).

## Session 150 — 2026-07-18 — YT-retrieval interlude: /watch-youtube designed, v0 built + eval'd (c998cc8)

**Outcome:** the Nick-ruled interlude closed same-day — the Researcher's YouTube discovery capability is designed (history-only), empirically validated against Nick's own curation, and filed for build (IB-179). Nick's watch history is the discovery list: the skill automates collection, curation stays human, every triage gate is unchanged.

- **Design note** (`project-management/design-notes/2026-07-18-yt-retrieval-design.md`): channel-feed RSS monitoring was verified working, then **rejected** by Nick — "too noisy; I use myself as a curator; YT History is the list" — and recorded as a do-not-re-propose alternative (so no watch fields on authorities, no channel-ID backfill). History access rides `yt-dlp :ythistory` via Chrome cookies (the official Data API dropped watch history in 2016; Takeout cadence is ~2 months); high-water mark replaces date filtering (no watched-at timestamps); handoff appends to LINKS.md for `/link-intake`. Nick's unregistered-channel promotion mechanism: 2–3+ relevant hits from an unknown channel → gated authority-registration proposal. Positioning vs the PRD bulk-video-intake non-goal is stated in-note (Nick: no PRD edit).
- **v0 eval** (Nick-ruled method: build the initial version, run it, score against the hand-curated batch): 28/28 recall, 0 false negatives, 6 on-topic extras → Nick ruled keep-1 / skip-5 → **84% precision**; the keep (HF self-training-agents talk, `OV56RddyFuU`) is the skill's **first accepted catch**, appended to LINKS.md under the skill's dated marker. Filter bar codified **strict** with the six verdicts as SKILL.md calibration exemplars; dedup caught 2 already-ingested videos Nick had re-pasted by hand. Report: `operations/research-reports/2026-07-18-watch-youtube-eval.md` (regression bar for the build's pilot).
- **Tool:** `app/yt-watch/discover.py` (v0 seed for IB-179; raw history + watermark gitignored — personal data never committed; committed report names on-topic titles only).
- **Lesson L-12** captured: a design whose output has a checkable ground-truth corpus includes a v0 empirical eval before finalization (owning surface `/design-skill` — its Phase 5 audit is static-only today).
- **Next-session queue ruled by Nick** (carried in PROGRESS): full `/link-intake` of the 29-link batch → guide refresh (agent memories / harnesses / agentic systems) → portable JSON/YAML asset-description language (E3 pull-forward; foundations for port/reharness meta-skills) → `/meta-skill-author` eval-sophistication upgrade anchored on the DeepMind good-skills framework (Schmid talk, in the batch). E1 memory-spec drafting stays parked-cresting behind the queue.

## Session 149 — 2026-07-16→18 — memory-spec research corpus shipped; drafting stashed (23c5cde…e871c50)

**Outcome:** E1 `memory-spec`'s full research corpus is on disk and synthesized in `operations/plans/memory-spec-inputs/` — internal grounding (A), a 12-framework external memory survey (B), and a harness-landscape survey with harness×memory harmonization (C). Both of the scope's open unknowns are answered; spec drafting is deliberately stashed (moves uphill→cresting), with a Nick-ruled interlude next: design a YouTube-retrieval enhancement for the Researcher.

- **Phase A (3 parallel briefs, Librarian + 2 general):** A1 KB memory-cluster design brief (verdicts: per-actor AND shared over one scoped substrate; append-only run log as first accumulation surface; 6 gaps G1–G6); A2 empirical substrate inventory (`operations/self/` live store, frozen SL corpus 153 entries, report folders; `reflections/` + `feedback/` dormant); A3 per-actor memory requirements (Owner dispatch ledger, Researcher intake cursor, Codifier rubric calibration mis-routed to Nick's MEMORY.md, Librarian with no durable output surface). Phase B-as-gap-check was skipped as unjustified, then reopened at Nick's request as a full framework survey.
- **Phase B (11→12 Sonnet collectors + Opus synthesizer):** per-framework memory dossiers — mem0, OpenClaw, Hermes, Claude Code native, Codex, Archon, Letta+Supermemory, Memongo+Mempalace, process repos (BMAD/Superpowers/GSD), catch-all scan, escalated four-pack (OpenViking/CrewAI/LangGraph/AutoGen), and Beads (Nick-requested augmentation). Synthesis: 7 de facto consensus practices; both A1 verdicts confirmed+refined (directory isolation over filter scoping; append-only is 2-of-3 and working-memory-scoped, needs explicit supersession); 9-mode failure catalog; D-series recommendations; process memory confirmed as fifth type routing to E2 (Beads is its strongest infrastructure validation); coding-vs-personal class dimension added at Nick's request — hypothesis confirmed (oracle availability is the deepest difference; engine = coding-lifecycle wrapper around oracle-free content; D5 decay/demotion firms UP); 12-item KB corrections ledger → IB-178.
- **Phase C (5 Sonnet collectors + Opus synthesizer):** harness-landscape survey at Nick's request — H1 terminology/taxonomy (scaffolding predates harness; 6 orthogonal dimensions), H2 LLM-first (enforcement structural-not-advisory across all three vendors; Codex ships native scheduling, Claude Code doesn't), H3 code-first (shape-vs-steering refinement; Pydantic AI capability primitive), H4 workflow orchestrators (DeerFlow migrated out of the class), H5 meta-harness/agent-OS (three postures — wrap/own/generate; OS-ness inversely correlates with adoption; markdown+git validated by two independent lineages). Harmonization synthesis: ruled glossary; LLM-first + file-first-via-hooks is the engine's own evidence-backed pairing; **D8 amended** (fourth native CC gap: scheduling/wake) — no other D-item changed; **E1↔E4 coupling**: reflection cadence rides E4's wake/dispatch, spec must state the seam; engine posture = own-the-loop migrating to generate-from-spec (E4→E5), never wrap.
- **Ops notes:** Perplexity MCP was down/flaky for all collectors — every dossier fell back to first-party docs/WebSearch/`gh` and flags it in an evidence-quality appendix (refresh pass named). One synthesizer stall (stream watchdog) required a clean rerun two days later — surfaced honestly after a session gap; corpus was unaffected.

## Session 148 — 2026-07-16 — Phase 4 shipped: kernel docs approved, checkpoint #2 ruled (3a2578d)

**Outcome:** the engine has its kernel documents — `governance/constitution.md`, `governance/prd.md`, `governance/actors.md`, every section Nick-approved in a single sitting via the adapted `/vision-to-plan` — and plan checkpoint #2 is ruled: **harness-first** (E4 before E5). The remaining program is now the PRD's seven epics; Phase 3 (user manual) folded into the kernel compiler.

- **Constitution** — drawn selectively from CHARTER (which stays at workspace root per DD-105, no supersession; A1 ruling: the charter *was* the constitution's origin, but not all of it is constitution material). Vision scope narrowed from "agentic systems of any kind" to **single-operator, harness-agnostic** (Claude Code = first compile target, not the boundary). The charter's seven values carried unchanged; eight principles — five previously-unwritten rulings given constitutional form (route-then-compact, kernel-vs-state litmus, derived-not-authored, door-type delegation, positive-space governance). Permissions ruled **hybrid**: four constitutional gates up top, per-actor sets in actors.md. New vision content elicited mid-block: the kernel is the system's complete machine-readable **self-description**, and audits return an **efficacy description + portable remediation prompt** the asker runs in their own harness.
- **PRD** — 3 users (Nick-builder, the asker, downstream kernel consumer; presenter/evaluator demoted to constitution-level audience), 6 goals with inline tests, **7 epics** with binary ACs. Nick's elicitation restructured the set mid-gate: **E1 memory & knowledge layer** (per-actor memory surfaces; data accumulation → reflection → knowledge; extends IB-176's `/self-improve`) and **E2 task management layer** (per-actor work queues incl. Owner; ruled work-item contract; absorbs the DD/IB cleanup + task-queue study) slotted ahead of E3 YAML self-description (one YAML per agent + per harness, schema deferred), E4 harness, E5 kernel compiler, E6 audits-ship-remedies, E7 fitness loop. Goal 6 added by amendment: knowledge and action close the loop.
- **actors.md** — C1 ruled **four actors, full stop** (DD-82 model is kernel truth; today's dispositions-by-skill is the degenerate case). C3 elicited the operating end-state: **the Owner is the acting owner of the whole system**, waking the other actors in separate sessions off per-actor queues, signal-driven via file mediation; the human drops to periodic review-of-changes + occasional use, with a named stays-human seam (kernel supersessions, irreversible/outward acts, beneficiary role). Extends DD-108 to its endpoint.
- **Block 0 resolutions:** "attachés" = **taches** (`taches-cc-resources` — already watched + analyzed; the Phase 1 gap-check's only GAP dissolves to GROUNDED); "Division, to a degree" = transcription noise, struck. Both recorded in the direction note.
- **Block E:** milestone mapping approved (memory-layer → task-layer → self-description → harness → kernel-compiler; E6 floating; next unit of work `memory-spec`); **Phase 3 folded into E5** as the kernel's human-readable manual layer (no standalone doc); design-mode video-intake spec kept and filed as **IB-177** (off the blocker list). Plan updated with the full checkpoint-#2 outcome; per-block run record in the interview-structure design note.

## Session 147 — 2026-07-16 — vision-to-plan import + guide refresh (backfilled; closed without handoff) (e8c213f…04b460e)

**Outcome:** the Phase 4 toolchain landed (`/vision-to-plan` imported + adapted from CareerBuddy `ops-vision-to-plan`, with engine artifact chain and templates) and 9 stale guides were re-synthesized. A `/detect-drift` predicate flaw surfaced — the scanner permanently re-flags artifacts already re-extracted via `--update` because DD-117 preserves `extraction_date`; 6 redundant re-extraction subagents were dispatched and 7 erroneous date bumps reverted pre-commit. Lesson L-10 captured with the fix (currency baseline = max(extraction_date, last_change_report)); skill hardening not yet applied.
...
- **Orchestration note + fix:** the initial 7-agent parallel extraction wave hit the account's Fable-5 monthly spend ceiling mid-run; the recovery (hand-reconcile the mid-abort orphans, then resume on Opus in staggered 3-agent waves) is the pattern to repeat. Nick then ruled the forward fix in-conversation: **before spinning up a subagent fan-out, ask which model class to use if it isn't specified** — moving the spend commitment to before the launch. Saved as a standing agent behavior (auto-memory `feedback_ask_model_class_before_fanout`), not an engine-file change, since it governs orchestration behavior across contexts rather than any single engine surface.
