...
consumed_by:
  - "rules/separate-planning-and-implementation-sessions.md"
...
## Update — 2026-04-20
GStack's planning pipeline provides concrete token-cost quantification for multi-stage session separation: Office Hours ~170k, Spec Team ~200k, Auto-Plan ~200k — totaling ~600k tokens across 3 separate Claude Code sessions. The practitioner explicitly recommends ending each stage, writing a handoff markdown artifact, and starting a fresh session for the next stage: "Accuracy is so important. Whenever you finish like one stage, take this as one Claude session, break it down, save it in a handoff doc, and carry over in the next session." The handoff prompt is generated by the planning session itself (Claude writes the exact command to run in the next session), reducing the human burden of context reconstruction. This pattern extends the Archon observation: the 3-stage GStack pipeline shows the principle scaling to complex brownfield feature work, not just simple plan→implement splits.
