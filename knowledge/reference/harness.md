...
A **harness** is the whole-system shape MetaSystem authors and audits: a folder (or set of folders) that composes one or more agentic artifacts (skills, agents, prompts) into a coherent runtime that a consumer points at, deploys into, or runs. The harness is the unit `/audit-system` audits and the unit `/design-harness` constructs.
...
| **Composition layer** | The *pattern* whereby a MetaSystem operation composes IL operations. `/audit-system` instantiates the composition layer; the harness is what the composition operates on. |
| **Codebase** | A harness is the agentic shape; the codebase is what underlies it. `/audit-system` is a harness audit, not a code review (see `/code-review`, `/security-review`). |

## Composition

This concept doc is **primarily §Construction**. §Composition (the audit-time substrate — which IL guides fire on harness aspects) is maintained by IL in `librarian/harness.md` and is read by `/audit-system` indirectly (via the per-artifact dispatches to `/assess-*`).
...
| Whole-system harness invariants (what `/audit-system` checks beyond per-artifact dispatch) | MetaSystem | `/audit-system` design contract §"Whole-system invariants" — **empty in v1 per rule 11**; candidates listed there |
...
3. **Enumerate the owned abstractions.** List which IL-maintained artifacts (skill, agent, prompt) this harness will ship or compose. Each shape adds an audit/design dispatch. Use the consumer-abstractions-map's split as the inventory:
   ...
   |---|---|---|---|
   | SKILL.md | counts in `/audit-system` discovery | `/assess-skill` | `/design-skill` |
   ...
   Record the artifact count and shape mix. This is the input to the bin-packing step `/audit-system` will run (see SKILL.md §"Sizing" / §"Bin packing").
...
4. **Run the Safety-critical classification.** Apply this gate — analogous to the per-skill G9.I6 gate but at the harness level.
   ...
   Per rule-10/G9.I6 at the harness level: a safety-critical harness must encode HITL gating in its composition (confirm-before-act, dry-run mode, conversation-only default with explicit `--write` opt-in). `/audit-system` ships this discipline: `--write` is off by default; safety-critical action requires consumer opt-in. Authoring a safety-critical harness without HITL gating produces a harness that will fail audit on first whole-system inspection.

5. **Specify the assessor binding (rule 10 — generator-assessor separation).** Every harness must have a paired assessor. Two cases:
   - **This harness is an assessor.** (Example: `/audit-system`.) Then it cannot also be the constructor for the same shape. Its constructive peer is a separate `/design-*` harness (rule 10).
   - **This harness is a constructor.** (Example: `/design-harness` when shipped.) Then it must delegate quality assessment to a separate assessor in fresh context (rule 10). For `/design-harness` the delegate is `/audit-system`.
...
6. **Specify the constructive-peer binding (rule 12 — audit/design symmetry).** Per rule 12, an abstraction that supports audit but not construction (or vice versa) is in symmetric debt — auditable-but-not-constructible (or the inverse). At the harness level this means:
   - If this harness is an assessor (`/audit-system`), its rule-12 peer is `/design-harness`. The peer must exist or be on the committed roadmap.
   - If this harness is a constructor (`/design-harness`), its rule-12 peer is `/audit-system`. Same.
...
## Input contract
<Table of inputs: required path/args + optional flags + defaults. Mirror /audit-system's shape.>
...
A harness spec missing any of `Plain-English purpose`, `Input contract`, `Owned abstractions`, `Safety-critical classification`, `Assessor binding`, `Constructive-peer binding`, `Output contract`, or `Boundaries` is not a complete harness — `/design-harness` (when shipped) will refuse to consider the spec stable, and `/audit-system` of a folder built from such a spec will surface the gaps as structural-incompleteness findings.
...
- **Stay one harness when:** the operation has multiple input modes but the same whole-system end-shape (e.g., `/audit-system <path>` vs. `/audit-system <path> --diff <prior>` — same operation, same output shape, one mode-flag).
...
Mistakes made while writing the harness spec. Distinct from `/audit-system`'s structural findings, which surface at audit time after the harness has artifacts authored.
...
2. **Assessor and constructor halves are both committed.** Per rule 12, the abstraction (here: harness) cannot be audit-only or design-only without filing the gap as debt. `/audit-system` shipped (audit half); `/design-harness` queued (design half) — symmetric.
...
Walk every `/audit-system` invariant and confirm §Construction tells the builder how to satisfy it. This is the rule-12 self-check; failures surface §Construction gaps to backfill.

| `/audit-system` check (audit-time) | §Construction satisfier (design-time) | Symmetric? |
|---|---|---|
...
| Whole-system summary synthesis (at-a-glance + critical findings + common findings + invariants) | Template skeleton §"Output contract" requires the harness to specify output shape; audit harnesses' shape mirrors `/audit-system`'s three artifacts | ✅ |
...
| Audit-trail one-line append to `audit-reports/runs.md` on `--write` | No symmetric design-time gate (mechanic of audit-time writes); construction-time concern is "the harness writes are reversible and audit-trail-emitting" — captured under safety-critical HITL gating | ✅ |
...
`/design-harness` (when shipped) will cite this concept doc by section anchor and IL substrate by file path. The audit-time substrate composed via `/audit-system` cites IL `librarian/*.md` per IL convention.
